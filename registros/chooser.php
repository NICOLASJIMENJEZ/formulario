<?php
// Simple chooser with three options: Formulario (public), Registros (protected), Registros por Programa (protected)
?>
<!doctype html>
<html lang="es">
<head>
    ...
    <style>
        ...
        /* Center droplet (gota de agua) and corner escudo */
        .escudo-center{
            position:absolute; left:50%; top:48%; transform:translate(-50%,-50%);
            width:420px; height:420px; background-repeat:no-repeat; background-position:center; background-size:contain;
            border-radius:50%;
            opacity:0.12; pointer-events:none; z-index:0; mix-blend-mode:screen;
            box-shadow: inset 0 -18px 60px rgba(255,255,255,0.06), inset 0 10px 30px rgba(0,0,0,0.02);
        }
        .escudo-corner{
            position:absolute; right:18px; top:12px; width:140px; height:140px; background-repeat:no-repeat; background-position:center; background-size:contain;
            opacity:1; pointer-events:none; z-index:1; filter: none; drop-shadow(0 2px 6px rgba(0,0,0,0.08));
        }
        /* Placeholder inside corner escudo */
...
<body>
        <div class="container py-5 wrap">
            <div class="escudo-center" aria-hidden="true" style="background-image: radial-gradient(circle at 35% 30%, rgba(255,255,255,0.85), rgba(255,255,255,0.35) 25%, rgba(255,255,255,0) 52%), url('/formulario/registros/escudo.png'); background-size: contain, contain; background-position: center, center; background-repeat: no-repeat, no-repeat; background-blend-mode: normal;"></div>
            <div class="escudo-corner" aria-hidden="true" style="background-image:url('/formulario/registros/escudo.png')">
                <div class="fallback">ESCUDO</div>
            </div>
        <h3 class="text-center mb-4">Selecciona una opción</h3>
        <div class="choice">
            <div class="card card-choice" role="button" onclick="location.href='/formulario/index.php'">
                <div class="card-body">
            ...
            <div class="card card-choice" role="button" onclick="location.href='/formulario/registros/login.php?goto=/formulario/registros/registros.php'">
                <div class="card-body">
                    <div class="card-ico"><i class="fa-solid fa-list-check"></i></div>
                    <h5>Registros</h5>
                    <p>Ver registros (se requiere contraseña)</p>
                </div>
            </div>

            <div class="card card-choice" role="button" onclick="location.href='/formulario/registros/login.php?goto=/formulario/registros/por_programa.php'">
                <div class="card-body">
    ...
    <script>
        async function clearAuthIfAny(){
            try{
                await fetch('/formulario/registros/clear_auth.php', {method:'POST'});
            }catch(e){ /* ignore */ }
        }

        // When user navigates back to chooser (bfcache/pageshow or popstate), clear auth so protected items require login
        window.addEventListener('pageshow', function(e){
            // pageshow persisted indicates bfcache restore in some browsers
            clearAuthIfAny();
        });
